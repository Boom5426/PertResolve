...
labels and returns the independent resolution axes, or use the
``pertresolve-resolution`` command for an ``.h5ad``.
...
``report``         the independent detection, identification, reproducibility-reference
                   and model-ranking axes together
...
mean separation the attainable score varies widely with the geometry of the configuration,
and it is ``rho2_nn_median`` rather than ``rho2`` that is most relevant to model-ranking
resolution.
"""
