...
``resolve_base``
    ...
    through ``--base`` or the ``PERTRESOLVE_DATA`` environment variable. It is never
...
_REPO_MARKERS = ("pertresolve", "results", "setup.py")

#: Environment variable naming the directory of processed per-gene arrays and the shared
#: scorer. The value must be supplied explicitly because the input workspace is not
#: redistributed with this repository.
ENV_VAR = "PERTRESOLVE_DATA"
...
ATLAS_ENV_VAR = "PERTRESOLVE_ATLAS_DIR"
...
def repo_root() -> Path:
    ...
    if missing:
        raise RuntimeError(
            ...
            "pertresolve/paths.py must stay inside the repository."
        )
    return candidate
...
def _repository_containing(path: Path) -> Path | None:
    """The repository ``path`` lies inside, or None.

    The guard below is about the *output* directory, so the repository has to be found from
    that directory rather than from where this module happens to sit. Resolving it from the
    module breaks the moment the package is copied to a compute box, and, worse, it would
    then stop protecting a repository checkout that the copied code was pointed at.
    """
...
def reject_repo_results(out: str | os.PathLike[str], *, flag: str = "--out") -> Path:
    ...
    owner = _repository_containing(resolved)
    if owner is None:
        # The destination is not inside any repository checkout, so there is no protected
        # tree to write into. This is the ordinary case on the compute box, where the
        # package is deployed outside the repository and results are written to a scratch
        # workspace; refusing to run there protects nothing.
        return resolved
    protected = owner / "results"
...
def add_harness_to_path(base: Path) -> Path:
    """Make the shared evaluation harness importable.

    The harness is now committed at ``scripts/analysis/harness.py`` and is the
    canonical definition of the pseudobulk target, the tie-aware PDS and the
    held-out design. It is preferred over any copy left in a compute workspace,
    so that a run cannot silently score against a stale local variant of the
    scorer. A workspace copy at ``<base>/unified`` is still accepted as a
    fallback, which keeps runs on machines predating this change working.

    The harness reads the per-gene arrays from ``base``, so ``base`` is exported
    through :data:`ENV_VAR` for the harness to resolve on first use.

    Args:
        base: the workspace returned by :func:`resolve_base`.

    Returns:
        The directory that was added to ``sys.path``.

    Raises:
        SystemExit: if no harness is found in either location.
    """
    os.environ.setdefault(ENV_VAR, str(base))
    committed = repo_root() / "scripts" / "analysis"
    if (committed / "harness.py").exists():
        if str(committed) not in sys.path:
