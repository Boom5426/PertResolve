...
```bash
python rep_grid.py    --base /path/to/VCCompass --out .
python am_analysis.py --base /path/to/VCCompass --out .
# or: export VCCOMPASS_BASE=/path/to/VCCompass
```
...
absent from `allele_perturb_bench.csv`, which is what `harness.split_vars` reads, so they never
