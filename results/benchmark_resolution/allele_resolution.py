...
``--base`` (or the ``PERTRESOLVE_DATA`` environment variable) points at the compute
workspace holding ``unified/harness.py`` and ``pertresolve_bench.csv``.
...
_parser.add_argument("--base", default=None,
                     help="processed-data directory (default: $PERTRESOLVE_DATA).")
...
# Checked before the inputs are looked for, so a --out that would overwrite a committed
# table is reported on any machine, not only one that already has the workspace.
