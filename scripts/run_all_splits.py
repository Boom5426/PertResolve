#!/usr/bin/env python
"""AllelePerturb-Eval v4: 10-metric enriched grid
...
def run_all():
    ...
    for sp in splits:
        ...
        for gene in genes:
            ...
            train_delta_mat = np.array([deltas[v] for v in train_vars if v in deltas])
            ...
            if esm_data is not None:
                for v in list(train_vars | test_vars):
                    key = f"{gene}__{v}"
                    if key in esm_data:
            ...
            heldout_list = [v for v in test_vars if v in deltas]
            train_list = [v for v in train_vars if v in deltas]
