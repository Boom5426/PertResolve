...
_ap.add_argument("--n-perm", type=int, default=50,
                 help="permutations of predictions across held-out variants, per method "
                      "(default: %(default)s)")
_ap.add_argument("--methods", default=None,
                 help="comma-separated subset of methods to score. The permutation cost is "
                      "linear in the number of methods, so raising --n-perm far enough to "
                      "resolve a corrected significance threshold is affordable only for the "
                      "few methods that came close to it. Omit to score every method.")
_args = _ap.parse_args()

BASE = resolve_base(_args.base)
OUT_DIR = reject_repo_results(_args.out)
OUT_DIR.mkdir(parents=True, exist_ok=True)
require_inputs(BASE / "unified" / "real_deltas.npz", BASE / "pertresolve_bench.csv")
...
if _args.methods:
    wanted = [m.strip() for m in _args.methods.split(",") if m.strip()]
    missing = [m for m in wanted if m not in methods]
    if missing:
        raise SystemExit(f"No prediction file for: {', '.join(missing)}")
    methods = [m for m in methods if m in wanted]
    print(f"scoring {len(methods)} of the available methods: {', '.join(methods)}")
