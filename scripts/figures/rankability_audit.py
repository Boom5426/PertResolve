...
PREP="/data/boom/NUS/floor_audit/prep"
OUT="/data/boom/NUS/floor_audit/results/floor_audit"; os.makedirs(OUT,exist_ok=True)
DATASET=sys.argv[1]
K=int(os.environ.get('K','50'))
NJOBS=int(os.environ.get('NJOBS','20'))

d=pickle.load(open(f"{PREP}/{DATASET}.pkl","rb"))
LEVEL=d["level"]
...
df.to_csv(f"{OUT}/{DATASET}_rankability.csv",index=False)
