...
The build and evaluation profiles come from disjoint cells, so the best predictor's ceiling
is the replicate ceiling rather than a perfect score. A predictor built from the cells it is
scored against would score 1.0 by matching its own noise and would say nothing.
...
near the ceiling and are again indistinguishable, so recovery falls. Measured on dialled
configurations it peaks around a ceiling of 0.705 and declines after
(``docs/RESULT_RESOLUTION_LAW_2026-08-04.md``). Nothing here should be inverted into a
design target without checking which side of that peak the data sit on.
"""
...
class RecoveryResult:
    ...
    def describe(self) -> str:
        ...
        return (f"orders predictors differing by {self.smallest_resolved_gap:.3f} in quality "
                f"(P(full order) = {self.p_correct_order:.2f}, "
                f"P(best chosen) = {self.p_correct_winner:.2f})")


def graded_predictor_scores(build: np.ndarray, truth: np.ndarray,
                            weights=DEFAULT_WEIGHTS, *,
                            toward: str = "shuffled") -> np.ndarray:
...
def _per_perturbation_pds(prediction: np.ndarray, truth: np.ndarray) -> np.ndarray:
    ...
    for i in range(dist.shape[0]):
        ...
        scores[i] = 1.0 - (less + (eq - 1) / 2.0) / (n - 1)
    return scores
